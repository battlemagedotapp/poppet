...
export type ChartConfig = Record<
	...
	} & ({ color?: string; theme?: never } | { color?: never; theme: { dark: string } })
>
		...
		>
			...
			<div className="grid gap-1.5">
				{visiblePayload.map((item, index) => {
					...
					const itemKey = `${key}-${String(item.value ?? "")}-${String(indicatorColor ?? "")}`
	...
	return (
		...
		>
			{visiblePayload.map((item) => {
				...
				const itemConfig = getPayloadConfigFromPayload(config, item, key)
				const itemKey = `${key}-${String(item.value ?? "")}-${String(item.color ?? "")}`
